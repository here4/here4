...
persistentGamepads is0 gs0 =
    let
        ...
        remap ids0 gs0 =
            case ( ids0, gs0 ) of
                ( [], _ ) ->
                    catMaybes (List.map (Maybe.map getId) gs0)

                ( is, [] ) ->
                    is

                ( i :: is, g :: gs ) ->
                    Maybe.withDefault i (Maybe.map getId g) :: remap is gs
...
gamepadToButtons gamepad =
    case gamepad.buttons of
        a :: b :: x :: y :: lb :: rb :: l :: r :: back :: start :: _ ->
            ...
            , bX = x.pressed
            , bY = y.pressed
            , bLeftBumper = lb.pressed
            , bRightBumper = rb.pressed
            , bBack = back.pressed
            , bStart = start.pressed
            }

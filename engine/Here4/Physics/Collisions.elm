module Here4.Physics.Collisions exposing (collisions)

import Array
import Here4.Body exposing (Massive, Spherical, Moving)
import Math.Vector3 as V3
import Math.Vector3 exposing (Vec3, vec3)
import Time exposing (Time)


type alias TimeLeft a =
    ( a, Float )
...
timeStep ( x, timeLeft ) =
    ( { x | position = V3.add x.position (V3.scale timeLeft x.velocity) }, timeLeft )
...
updatePairs f arr0 =
    let
        ...
        update m n arr =
            case g (Array.get m arr) (Array.get n arr) of
                Nothing ->
                    arr

                Just ( vm, vn ) ->
                    Array.set m vm << Array.set n vn <| arr

        row m arr =
            List.foldl (update m) arr (List.range (m + 1) (len - 1))
    in
        List.foldl row arr0 (List.range 0 (len - 1))
...
collide dt ( a, ta ) ( b, tb ) =
    let
        ...
        centerDisplacement =
            V3.sub b.position a.position

        centerDistance =
            V3.length centerDisplacement
                    ...
                    case passBy of
                        ...
                        False ->
                            let
                                ...
                                ttt =
                                    square sumRadii - periSquared

                                -- The distance A actually travels before collision, taking radii into account
                                distanceToCollisionA =
                                    distanceToPeriA - sqrt ttt

                                -- Have we even moved far enough to collide?
                                notFarEnough =
                                    V3.length relativeMovement < distanceToCollisionA
                            in
                                case notFarEnough of
                                    ...
                                    False ->
                                        let
                                            ...
                                            relativeCollision =
                                                V3.scale distanceToCollisionA normRelMovement

                                            -- Time taken before collision == collisionDelta * dt
                                            collisionDelta =
                                                V3.length relativeCollision / V3.length relativeMovement

                                            -- Movement vector of A to point of collision
                                            collisionVectorA =
                                                V3.scale (collisionDelta * dt) a.velocity

                                            -- Movement vector of B to point of collision
                                            collisionVectorB =
                                                V3.scale (collisionDelta * dt) b.velocity

                                            timeLeft =
                                                (1.0 - collisionDelta) * dt

                                            -- movedA = { a | pos = V3.add a.pos collisionVectorA, timeLeft = timeLeft }
                                            movedA =
                                                { a | position = V3.add a.position collisionVectorA }

                                            -- movedB = { b | pos = V3.add b.pos collisionVectorB, timeLeft = timeLeft }
                                            movedB =
                                                { b | position = V3.add b.position collisionVectorB }

                                            -- Projection of movement onto new centerDisplacement
                                            centerDisplacement_ =
                                                V3.sub movedA.position movedB.position

                                            nnn =
                                                V3.normalize centerDisplacement_

                                            projA =
                                                V3.dot a.velocity nnn

                                            projB =
                                                V3.dot b.velocity nnn

                                            optimizedP =
                                                (2.0 * (projA - projB)) / (a.mass + b.mass)

                                            newVelA =
                                                V3.sub a.velocity (V3.scale (optimizedP * b.mass) nnn)

                                            newVelB =
                                                V3.add b.velocity (V3.scale (optimizedP * a.mass) nnn)

                                            -- XXX: Subsequent movement during this timestep should only move for
                                            -- the remaining time (1.0-collisionDelta)*dt
                                            collidedPair =
                                                ( ( { movedA | velocity = newVelA }, timeLeft ), ( { movedB | velocity = newVelB }, timeLeft ) )
                                        in
                                            Just collidedPair
...
collisions dt =
    List.map (stripTimeStep << timeStep)
        << Array.toList
        << updatePairs (collide dt)
        << Array.fromList
        << List.map (setTimeLeft dt)

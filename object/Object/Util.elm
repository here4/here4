...
transformBounds f (origin, dimensions) =
    let
        ...
        tMinV = f origin
        tMaxV = V3.add tMinV (f dimensions)
        (x1, y1, z1) = V3.toTuple tMinV
        (x2, y2, z2) = V3.toTuple tMaxV
    in
        bounds
            [ vec3 x1 y1 z1
            , vec3 x1 y1 z2
...
scaleToVec3 rotatedModelDimensions scale =
    let 
        (dx, dy, dz) = V3.toTuple rotatedModelDimensions
    in
        case scale of
            Scale3 fx fy fz ->
                vec3 fx fy fz
...
offsetToVec3 modelToWorld worldDimensions offset =
    let 
        (dx, dy, dz) = V3.toTuple worldDimensions
    in
        case offset of
            ModelSpace mx my mz ->

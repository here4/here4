...
type alias VertVTN =
    { position : Vec3, texCoord : Vec3, normal : Vec3 }
...
toVTN f =
    case f of
        ...
        FaceVertVN vn ->
            { position = vn.position, normal = vn.normal, texCoord = vec3 0 0 0 }

        FaceVertV v ->
            { position = v.position, normal = vec3 0 1 0, texCoord = vec3 0 0 0 }

        FaceVertVT vt ->
            { position = vt.position, normal = vec3 0 1 0, texCoord = vt.texCoord }
            ...
            [ base, power ] ->
                let
                    ...
                    piR =
                        String.toInt power
                in
                    Result.map2 (\bf pi -> bf * (myPow 10.0 pi)) bfR piR
...
isVertexLine s =
    Maybe.withDefault False <| Maybe.map (\w -> w == "v") (List.head <| String.words s)


isVtLine =
    (\s -> String.startsWith "vt" s)


isVnLine =
    (\s -> String.startsWith "vn" s)


isFaceLine =
    (\s -> String.startsWith "f" s)
...
lineToVert line =
    case (String.words line) of
        [ "v", v1, v2, v3 ] ->
            Maybe.map3 vec3 (parseFloat v1) (parseFloat v2) (parseFloat v3)
...
lineToVt line =
    case (String.words line) of
        ...
        [ "vt", v1, v2 ] ->
            Maybe.map2 (\x y -> vec3 x y 0.0) (parseFloat v1) (parseFloat v2)
...
lineToFace vArr vtArr vnArr line =
    let
        ...
        parse =
            parseFaceVert vArr vtArr vnArr
    in
        Maybe.withDefault [] <|
            case (String.words line) of
                [ "f", f1, f2, f3 ] ->
                    Maybe.map3 (\p1 p2 p3 -> [ ( p1, p2, p3 ) ]) (parse f1) (parse f2) (parse f3)

                [ "f", f1, f2, f3, f4 ] ->
                    Maybe.map4 (\p1 p2 p3 p4 -> [ ( p1, p2, p3 ), ( p2, p3, p4 ) ]) (parse f1) (parse f2) (parse f3) (parse f4)
...
parseFaceVert : Array.Array Vec3 -> Array.Array Vec3 -> Array.Array Vec3 -> String -> Maybe FaceVert
parseFaceVert vArr vtArr vnArr str =
    case String.split "//" str of
        [ v, n ] ->
            Maybe.map FaceVertVN (Maybe.map2 VertVN (deIndexVert vArr v) (deIndexVert vnArr n))

        --vertex and normal
        [ s ] ->
            case String.split "/" s of
                [ v ] ->
                    Maybe.map (FaceVertV << VertV) (deIndexVert vArr v)

                --only vertex
                [ v, t ] ->
                    Maybe.map FaceVertVT <| Maybe.map2 VertVT (deIndexVert vArr v) (deIndexVert vtArr t)

                --vertex and tex coords
                [ v, t, n ] ->
                    Maybe.map FaceVertVTN <| Maybe.map3 VertVTN (deIndexVert vArr v) (deIndexVert vtArr t) (deIndexVert vnArr n)
...
deIndexVert vArr s =
    Maybe.andThen (Result.toMaybe (String.toInt s)) (\i -> Array.get (i - 1) vArr)


normalize : List Vec3 -> List Vec3
normalize list =
    let
        findMinMax minCoord maxCoord list =
            case list of
                [] ->
                    ( minCoord, maxCoord )
                ...
                v :: vs ->
                    let
                        ...
                        minCoord' =
                            min minCoord (min r.x (min r.y r.z))

                        maxCoord' =
                            max maxCoord (max r.x (max r.y r.z))
                    in
                        findMinMax minCoord' maxCoord' vs

        ( minCoord, maxCoord ) =
            Debug.log "findMinMax" <| findMinMax 1000000 -1000000 list
        ...
        normV v =
            let
                r =
                    V3.toRecord v
            in
                vec3 (norm r.x) (norm r.y) (norm r.z)
    in
        List.map normV list
...
recenter list =
    let
        ...
        findMinMax minX maxX minY maxY minZ maxZ list =
            case list of
                [] ->
                ...
                v :: vs ->
                    let
                        ...
                        maxZ' =
                            max maxZ r.z
                    in
                        findMinMax minX' maxX' minY' maxY' minZ' maxZ' vs

        ( minX, maxX, minY, maxY, minZ, maxZ ) =
            Debug.log "findMinMax" <| findMinMax fltMax fltMin fltMax fltMin fltMax fltMin list
...
parseObj inFile =
    let
        ...
        vLines =
            List.filter isVertexLine lines

        vertices =
            Array.fromList <| normalize <| recenter <| List.filterMap lineToVert vLines

        vtLines =
            List.filter isVtLine lines

        texCoords =
            Array.fromList <| List.filterMap lineToVt vtLines

        vnLines =
            List.filter isVnLine lines

        normals =
            Array.fromList <| List.filterMap lineToVn vnLines

        fLines =
            List.filter isFaceLine lines

        faces =
            List.concat <| List.map (lineToFace vertices texCoords normals) fLines
    in
        faces


mesh inFile =
    Triangle <| List.map (\( a, b, c ) -> ( toVTN a, toVTN b, toVTN c )) <| parseObj inFile
...
loadMesh infile =
    Signal.map mesh objMailbox.signal


objThing : Mesh VertVTN -> Perception -> List Entity
objThing mesh p =
    [ entity vertexShader fragmentShader mesh { view = p.viewMatrix } ]


loadObj infile =
    Signal.map objThing (loadMesh infile)
...
loadJeepMesh =
    Signal.map mesh objJeepMailbox.signal
...
loadJeep infile =
    Signal.map jeepThing loadJeepMesh

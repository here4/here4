...
type alias Model = Maybe (Moving Body)
...
bodies model = case model of
    Just body -> [toBody body]
    Nothing   -> []

camera : Model -> Maybe Camera
camera model = Maybe.map bodyCamera model

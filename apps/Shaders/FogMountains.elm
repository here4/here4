...
vec2 map(vec3 p, int octaves)
{
  ...
  if (h < 0.0) { // no need to check for trees at higher elevations
    ...
    f = (0.1*f) - 0.3; // limit the altitude of trees
    d = p.y - f;
    if (d < dMin) {
      dMin = d;
      mID = 1.0;
...
vec3 render(vec3 ro, vec3 rd)
{
  ...
  if (res.y > -0.5 && res.y < 0.5) {
    ...
    // layer noise (to produce lighter color bands of rock)
    float n = 0.5 * (snoise(pos.xy*vec2(2.0, 40.0))+1.0);
    // rock layers should show most where normals are NOT straight up
  ...
  float fog = exp(-0.01 * res.x*res.x); // exponential fog equation
...
void fogMountains(vec2 tc)
{
  ...
  // camera
  float x = 5.0 + (0.2*iGlobalTime);
  float y = 0.0;
  float z = 0.0 + 3.0*sin(0.1*iGlobalTime);

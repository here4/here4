...
fly eyeLevel inputs motion =
    let
        ...
        dv = V3.scale (5 * thrust * inputs.dt) <| orient V3.k
        du = V3.scale (2 * thrust * inputs.dt) <| orient V3.j
        dv_ = add dv du
...
keepWithinbounds ground motion = { motion | position = ground.bounds motion.position }
